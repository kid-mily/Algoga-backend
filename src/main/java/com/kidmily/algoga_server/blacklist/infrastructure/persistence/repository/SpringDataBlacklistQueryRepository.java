...
public interface SpringDataBlacklistQueryRepository extends JpaRepository<BlacklistJpaEntity, Long> {
    ...
    @Query(value = """
            ...
            WHERE u.is_deleted = false
              ...
              AND (:keyword = '' OR u.username LIKE CONCAT('%', :keyword, '%') OR u.nickname LIKE CONCAT('%', :keyword, '%') OR u.email LIKE CONCAT('%', :keyword, '%'))
            """,
            countQuery = """
    ...
    @Query(value = """
            SELECT 
                ...
                1 AS isBlacklisted
            FROM users u
            INNER JOIN blacklists bl ON u.user_id = bl.user_id
            WHERE u.is_deleted = false
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    Optional<BlacklistUserQueryProjection> findUserDetailById(@Param("userId") Long userId);
}