...
public class FriendController {
    ...
    public ApiResponse<List<FriendResponse>> getFriends(@AuthenticationPrincipal CustomUserDetails user) {
        List<FriendResponse> response = queryUseCase.getFriends(user.getUser().getId());
    ...
    public ApiResponse<List<FriendResponse>> getReceivedRequests(@AuthenticationPrincipal CustomUserDetails user) {
        List<FriendResponse> response = queryUseCase.getReceivedRequests(user.getUser().getId());
    ...
    public ApiResponse<FriendResponse> searchUserByCode(@RequestParam String code) {
        FriendResponse response = queryUseCase.searchUserByCode(code);
        return ApiResponse.success("USER_SEARCH_SUCCESS", "유저를 검색했습니다.", response);
    }

    @Operation(summary = "친구 추가 요청")
    @PostMapping("/friends/requests")
    public ApiResponse<Void> sendFriendRequest(
            @AuthenticationPrincipal CustomUserDetails userDetails, // null일 수 있음
            @RequestBody CreateFriendRequest request) {

        // 1. 로그인 안 된 사용자인지 확인 (방어 코드)
        if (userDetails == null) {
            throw new FriendException(FriendErrorCode.USER_NOT_FOUND); // 혹은 401 에러용 별도 예외
        }

        // 2. 정상적인 경우만 로직 수행
        commandUseCase.sendFriendRequest(userDetails.getUser().getId(), request.targetUserCode());
    ...
    @Operation(summary = "유저 차단", description = "상대방 유저를 차단합니다. 차단된 유저와는 친구 관계가 해제되며, 차단 해제 전까지는 친구 추가를 할 수 없습니다.")
    @PostMapping("/friends/blocks")
    public ApiResponse<Void> blockUser(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @RequestBody CreateFriendRequest request) { // 기존 DTO 재활용
        if (userDetails == null) throw new FriendException(FriendErrorCode.USER_NOT_FOUND);
        commandUseCase.blockUser(userDetails.getUser().getId(), request.targetUserCode());
        return ApiResponse.success("USER_BLOCK_SUCCESS", "유저를 차단했습니다.");
    }

    @Operation(summary = "유저 차단 해제", description = "차단했던 유저의 차단 상태를 해제하여 다시 관계를 맺을 수 있는 상태로 변경합니다.")
