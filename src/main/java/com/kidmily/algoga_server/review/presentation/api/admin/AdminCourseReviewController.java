...
import com.kidmily.algoga_server.review.presentation.request.admin.UpdateCourseReviewVisibilityRequest;
import com.kidmily.algoga_server.review.presentation.response.AdminCourseReviewResponse;
...
public class AdminCourseReviewController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<PageResponse<AdminCourseReviewResponse>>> getReviews(
            @Parameter(description = "강의 ID", example = "57")
            @PathVariable Long courseId,

            @ParameterObject @PageableDefault(size = 10) Pageable pageable
    ) {
        Page<AdminCourseReviewResponse> response = courseReviewUseCase.getAdminReviews(courseId, pageable)
